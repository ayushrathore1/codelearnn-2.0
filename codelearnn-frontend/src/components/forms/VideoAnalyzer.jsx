...
const VideoAnalyzer = ({ onAnalysisComplete }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to analyze video. Please try again.';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const getRecommendationStyle = (recommendation) => {
    const styles = {
      'strongly_recommend': { bg: 'bg-green/20', text: 'text-green', label: '✅ Strongly Recommended' },
      'recommend': { bg: 'bg-green/15', text: 'text-green', label: '👍 Recommended' },
      'neutral': { bg: 'bg-yellow-500/20', text: 'text-yellow-400', label: '⚖️ Neutral' },
      'caution': { bg: 'bg-orange-500/20', text: 'text-orange-400', label: '⚠️ Use with Caution' },
      'avoid': { bg: 'bg-red-500/20', text: 'text-red-400', label: '❌ Not Recommended' }
    };
  ...
  const getConfidenceStyle = (confidence) => {
    switch (confidence) {
      case 'high': return { bg: 'bg-green/10', text: 'text-green', border: 'border-green/30', label: 'High Confidence' };
      case 'low': return { bg: 'bg-red-500/10', text: 'text-red-400', border: 'border-red-500/30', label: 'Low Confidence' };
      default: return { bg: 'bg-blue-500/10', text: 'text-blue-400', border: 'border-blue-500/30', label: 'Medium Confidence' };
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
          ...
          >
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="text-white font-heading font-semibold">Analyzing Video Honestly</p>
      ...
      <AnimatePresence>
        {result && (
          ...
          >
              ...
              >
                  ...
                  <div className="flex items-start gap-4">
                    <div className="relative w-32 aspect-video rounded overflow-hidden flex-shrink-0">
                      ...
                      <div className="absolute inset-0 bg-navy/60 flex items-center justify-center">
                        <FontAwesomeIcon icon={faList} className="text-white text-lg" />
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {result.evaluation?.evaluationConfidence && (
                          <span className={`px-2 py-0.5 rounded text-xs font-mono border ${getConfidenceStyle(result.evaluation.evaluationConfidence).bg} ${getConfidenceStyle(result.evaluation.evaluationConfidence).text} ${getConfidenceStyle(result.evaluation.evaluationConfidence).border}`}>
                            <FontAwesomeIcon icon={faShieldAlt} className="mr-1" />
                            {getConfidenceStyle(result.evaluation.evaluationConfidence).label}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-3 mt-2 text-xs font-mono text-light-slate">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {result.aggregateStats?.totalViews?.toLocaleString()} total views
                        </span>
                      </div>
                    </div>
                    <div className="flex-shrink-0 text-center">
                      <CodeLearnnScore 
                        ...
                        showLabel={true}
                      />
                      <p className="text-xs font-mono text-slate mt-1">Avg Score</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 border-b border-lightest-navy bg-navy/30">
                  <p className="text-light-slate text-sm">
                    <FontAwesomeIcon icon={faInfoCircle} className="text-green mr-2" />
                  ...
                  {result.aggregateStats?.nonProgrammingVideoCount > 0 && (
                    <div className="mt-3 p-2 bg-orange-500/10 border border-orange-500/30 rounded text-xs">
                      <span className="text-orange-400">
                        ⚠️ {result.aggregateStats.nonProgrammingVideoCount} of {result.playlistData.analyzedCount} analyzed videos are not programming tutorials
                      </span>
                    </div>
                  )}
                </div>
                ...
                {/* Red Flags Warning */}
                {result.evaluation?.redFlags && result.evaluation.redFlags.length > 0 && (
                ...
                {result.evaluation?.breakdown && (
                  <div className="border-t border-lightest-navy p-4">
                    <h4 className="text-slate font-mono text-xs mb-3 flex items-center gap-2">
                      <FontAwesomeIcon icon={faChartLine} />
                      AVERAGE SCORE BREAKDOWN (across {result.aggregateStats?.programmingVideoCount} programming videos)
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                      {[
                        { label: 'Content', value: result.evaluation.breakdown.contentQuality, max: 10 },
                        { label: 'Teaching', value: result.evaluation.breakdown.teachingClarity, max: 10 },
                        { label: 'Practical', value: result.evaluation.breakdown.practicalValue, max: 10 },
                        { label: 'Up-to-Date', value: result.evaluation.breakdown.upToDateScore, max: 10 },
                        { label: 'Sentiment', value: result.evaluation.breakdown.commentSentiment, max: 10 },
                        { label: 'Engagement', value: result.evaluation.breakdown.engagement, max: 100 }
                      ].map((item, idx) => (
                        <div key={idx} className="text-center">
                          <div className="text-lg font-mono font-bold text-white">
                            {item.value || 0}<span className="text-slate text-xs">/{item.max}</span>
                          </div>
                          <div className="text-[10px] text-slate font-mono">{item.label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {result.evaluation?.strengths?.map((strength, idx) => (
                        <li key={idx} className="text-light-slate text-xs flex items-start gap-2">
                          <FontAwesomeIcon icon={faCheck} className="text-green mt-0.5 text-[10px]" />
                          {strength}
                        </li>
                      ))}
                      {(!result.evaluation?.strengths || result.evaluation.strengths.length === 0) && (
                        <li className="text-slate text-xs italic">No common strengths identified</li>
                      )}
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {result.evaluation?.weaknesses?.map((weakness, idx) => (
                        <li key={idx} className="text-light-slate text-xs flex items-start gap-2">
                          <FontAwesomeIcon icon={faTimes} className="text-orange-400 mt-0.5 text-[10px]" />
                          {weakness}
                        </li>
                      ))}
                      {(!result.evaluation?.weaknesses || result.evaluation.weaknesses.length === 0) && (
                        <li className="text-slate text-xs italic">No common weaknesses found</li>
                      )}
                    </ul>
                  </div>
                </div>
                ...
                <div className="border-t border-lightest-navy p-4">
                  <h4 className="text-slate font-mono text-xs mb-3 flex items-center gap-2">
                    <FontAwesomeIcon icon={faPlay} />
                    ANALYZED VIDEOS ({result.evaluation?.videoAnalyses?.length || 0})
                  </h4>
                  <div className="space-y-2">
                    {result.evaluation?.videoAnalyses?.map((video, idx) => (
                      ...
                      >
                        <span className="text-slate text-xs font-mono w-5">{idx + 1}.</span>
                        ...
                        <div className="flex-shrink-0">
                          {video.isProgrammingTutorial ? (
                            <CodeLearnnScore score={video.score} size="sm" showLabel={false} />
                          ) : (
            ...
            {!result.isPlaylist && evaluation && evaluation.isProgrammingTutorial === false && (
              <motion.div
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-light-slate text-sm mb-3">
                      This video doesn't appear to be about programming, coding, or tech education.
                    </p>
                    <div className="bg-navy/50 rounded p-3 mb-3">
                      <p className="text-slate text-xs font-mono mb-1">Detected Category:</p>
                      <p className="text-white font-semibold capitalize">
                        {evaluation.detectedCategory || 'Non-programming content'}
                      </p>
                    </div>
                    ...
                    <div className="mt-4 flex gap-3">
                      <a
                        href={`https://www.youtube.com/watch?v=${
                          result.isNew ? result.videoData?.youtubeId : result.resource?.youtubeId
                        }`}
                        target="_blank"
                        rel="noopener noreferrer"
            ...
            <div className="bg-light-navy rounded-lg border border-lightest-navy overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {evaluation?.recommendation && (
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-mono border ${getRecommendationStyle(evaluation.recommendation).bg} ${getRecommendationStyle(evaluation.recommendation).text} ${getRecommendationStyle(evaluation.recommendation).border}`}>
                        {getRecommendationStyle(evaluation.recommendation).label}
                      </div>
                    )}
                    {evaluation?.evaluationConfidence && (
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-mono border ${getConfidenceStyle(evaluation.evaluationConfidence).bg} ${getConfidenceStyle(evaluation.evaluationConfidence).text} ${getConfidenceStyle(evaluation.evaluationConfidence).border}`}>
                        <FontAwesomeIcon icon={faShieldAlt} className="text-xs" />
                        {getConfidenceStyle(evaluation.evaluationConfidence).label}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex-shrink-0 text-center">
                  ...
                  {evaluation?.qualityTier && (
                    <p className={`text-xs font-mono mt-1 capitalize ${getQualityTierStyle(evaluation.qualityTier)}`}>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    ...
                    {(!evaluation?.strengths || evaluation.strengths.length === 0) && (
                      <li className="text-slate text-xs italic">No specific strengths identified</li>
                    )}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    ...
                    {(!evaluation?.weaknesses || evaluation.weaknesses.length === 0) && (
                      <li className="text-slate text-xs italic">No significant weaknesses found</li>
                    )}
                  </ul>
                </div>
              </div>
              ...
              {evaluation?.breakdown && (
                <div className="border-t border-lightest-navy p-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {[
                      { label: 'Content', value: evaluation.breakdown.contentQuality, max: 10 },
                      { label: 'Teaching', value: evaluation.breakdown.teachingClarity, max: 10 },
                      { label: 'Practical', value: evaluation.breakdown.practicalValue, max: 10 },
                      { label: 'Up-to-Date', value: evaluation.breakdown.upToDateScore, max: 10 },
                      { label: 'Sentiment', value: evaluation.breakdown.commentSentiment, max: 10 },
                      { label: 'Engagement', value: evaluation.breakdown.engagement, max: 100 }
                    ].map((item, idx) => (
                      <div key={idx} className="text-center">
                        <div className="text-lg font-mono font-bold text-white">
                          {item.value || 0}<span className="text-slate text-xs">/{item.max}</span>
                  ...
                  {/* Penalties */}
                  {(evaluation.penalties?.outdated > 0 || evaluation.penalties?.confusion > 0) && (
                    <div className="mt-3 pt-3 border-t border-lightest-navy flex gap-4 text-xs font-mono">
                      {evaluation.penalties?.outdated > 0 && (
                        <span className="text-orange-400">
                          ⚠️ Outdated penalty: -{evaluation.penalties.outdated}
                        </span>
                      )}
                      {evaluation.penalties?.confusion > 0 && (
                        <span className="text-orange-400">
                          ⚠️ Confusion penalty: -{evaluation.penalties.confusion}
                        </span>
                      )}
                    </div>
                  )}
              ...
              {/* Comment Analysis */}
              {evaluation?.commentAnalysis && evaluation.commentAnalysis.totalAnalyzed > 0 && (
                <div className="border-t border-lightest-navy p-4">
                  <h4 className="text-slate font-mono text-xs mb-2">
                    📊 Analyzed {evaluation.commentAnalysis.totalAnalyzed} comments - 
                    Sentiment: <span className={
                      evaluation.commentAnalysis.sentiment === 'very_positive' ? 'text-green' :
                      evaluation.commentAnalysis.sentiment === 'positive' ? 'text-green/80' :
                      evaluation.commentAnalysis.sentiment === 'negative' ? 'text-orange-400' :
                      evaluation.commentAnalysis.sentiment === 'very_negative' ? 'text-red-400' :
                      'text-yellow-400'
                    }>{evaluation.commentAnalysis.sentiment?.replace('_', ' ').toUpperCase()}</span>
                  </h4>
                  {evaluation.commentAnalysis.concerns && evaluation.commentAnalysis.concerns.length > 0 && (
                    <div className="mt-2">
                      <p className="text-orange-400 text-[10px] font-mono mb-1">Top concerns from comments:</p>
                      {evaluation.commentAnalysis.concerns.slice(0, 2).map((concern, idx) => (
                        <p key={idx} className="text-slate text-[11px] italic line-clamp-1">
                          "{concern}"
                        </p>
                      ))}
                    </div>
      ...
      {!result && !loading && (
        ...
        >
          <FontAwesomeIcon icon={faChartLine} className="text-4xl text-green/30 mb-4" />
          <p className="text-slate text-sm font-mono">
            Paste any YouTube <span className="text-green">video or playlist</span> URL to get an honest quality assessment
          </p>
          <p className="text-text-tertiary text-xs font-mono mt-2">
            We analyze comments, detect outdated content, and provide critical feedback - not just praise
          </p>
