...
const PostDetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        if (charchaUser) {
          const votesRes = await charchaVotesAPI.checkVotes('post', [response.data.post?._id || response.data._id]);
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      try {
        const response = await charchaCommentsAPI.getComments(post._id, { sort: 'best' });
        setComments(response.data.comments || []);
        
        // Check user's votes on comments
        if (charchaUser && response.data.comments?.length > 0) {
          const commentIds = getAllCommentIds(response.data.comments);
          if (commentIds.length > 0) {
            const votesRes = await charchaVotesAPI.checkVotes('comment', commentIds);
  ...
  const handleVote = async (voteType) => {
    ...
    try {
      ...
      setPost(prev => ({
        ...prev,
        voteScore: prev.voteScore + (userVote === voteType ? -voteType : voteType - userVote)
      }));
    } catch (err) {
          ...
          {post.mediaUrls && post.mediaUrls.length > 0 && (
            <div className="grid grid-cols-2 gap-3 mb-6">
              {post.mediaUrls.map((media, idx) => (
                <div key={idx} className="rounded-lg overflow-hidden bg-bg-elevated border border-border">
                  {media.type === 'image' && (
        ...
        <div className="card-bento p-4 mb-6">
          <div className="flex gap-3">
            <textarea
              ...
              placeholder={charchaUser ? "Add a comment..." : "Login to comment"}
